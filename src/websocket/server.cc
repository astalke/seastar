...
future<> server::do_accept_one(int which) {
    return _listeners[which].accept().then([this] (accept_result ar) mutable {
        ...
        (void)try_with_gate(_task_gate, [conn = std::move(conn)]() mutable {
            return conn->process().finally([conn = std::move(conn)] () mutable {
                return conn->close().handle_exception([] (const std::exception_ptr& e) {
                    wlogger.debug("Closing failed: {}", e);
...
future<> connection::read_loop() {
    return read_http_upgrade_request().then([this] {
        return when_all_succeed(_handler(_input, _output),
        ...
        wlogger.debug("Failed to process HTTP upgrade request: {}", e);
        shutdown();
